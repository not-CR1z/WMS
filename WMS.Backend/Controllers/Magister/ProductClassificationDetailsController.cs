...
namespace WMS.Backend.Controllers.Magister
{
    ...
    public class ProductClassificationDetailsController : Controller
    {
        private readonly IProductClassificationDetailUnitOfWork _unitOfWork;
        private readonly IValidateSession _validateSession;
        public ProductClassificationDetailsController(IProductClassificationDetailUnitOfWork unitOfWork, IValidateSession validateSession)
        {
        ...
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 13, "Read");
        ...
        public async Task<IActionResult> GetGenericSearchAsync([FromQuery] PaginationDTO pagination)
        {
            ...
            if (response.WasSuccess)
            {
                var List = (List<ProductClassificationDetail>)response.Result!;
                var ListReturn =
                    (from item in List
        ...
        [HttpPost]
        public virtual async Task<IActionResult> PostAsync(ProductClassificationDetail model)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 13, "Create");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 13, "Delete");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        [HttpPut]
        public virtual async Task<IActionResult> PutAsync(ProductClassificationDetail model)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 13, "Update");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        public virtual async Task<IActionResult> UploadAsync(List<ProductClassificationDetail> list)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 13, "Create");
            if (!AuthForm.WasSuccess)
            {
