...
namespace WMS.Backend.Controllers.Magister
{
    ...
    {
        private readonly IProductClassificationUnitOfWork _unitOfWork;
        private readonly IValidateSession _validateSession;
        public ProductClassificationsController(IProductClassificationUnitOfWork unitOfWork, IValidateSession validateSession)
        {
        ...
        public async Task<IActionResult> GetGenericSearchAsync([FromQuery] PaginationDTO pagination)
        {
            ...
            if (response.WasSuccess)
            {
                var List = (List<ProductClassification>)response.Result!;
                var ListReturn =
                    (from item in List
        ...
        [HttpPost]
        public virtual async Task<IActionResult> PostAsync(ProductClassification model)
        {
        ...
        [HttpPut]
        public virtual async Task<IActionResult> PutAsync(ProductClassification model)
        {
        ...
        public virtual async Task<IActionResult> UploadAsync(List<ProductClassification> list)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 13, "Create");
            if (!AuthForm.WasSuccess)
            {
