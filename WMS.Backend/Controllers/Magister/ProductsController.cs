...
namespace WMS.Backend.Controllers.Magister
{
    ...
    public class ProductsController : Controller
    {
        ...
        public ProductsController(IProductUnitOfWork unitOfWork, IValidateSession validateSession)
        {
        ...
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 15, "Read");
        ...
        [HttpPost]
        public virtual async Task<IActionResult> PostAsync(Product model)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 15, "Create");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        [HttpDelete("deleteclasificationasync/{id}")]
        public virtual async Task<IActionResult> DeleteClasificationAsync(long id)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 15, "Delete");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
            var response = await _unitOfWork.DeleteClasificationAsync(id);
            if (response.WasSuccess)
            {
                return NoContent();
            }
        ...
        [HttpPut]
        public virtual async Task<IActionResult> PutAsync(Product model)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 15, "Update");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        public virtual async Task<IActionResult> UploadAsync(List<Product> list)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 15, "Create");
            if (!AuthForm.WasSuccess)
            {
