...
namespace WMS.Backend.Controllers.Magister
{
    ...
    public class UMsController:Controller
    {
        ...
        public UMsController(IUMUnitOfWork unitOfWork, IValidateSession validateSession)
        {
        ...
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 14, "Read");
        ...
        public async Task<IActionResult> GetGenericSearchAsync([FromQuery] PaginationDTO pagination)
        {
            ...
            if (response.WasSuccess)
            {
                var List = (List<UM>)response.Result!;
                var ListReturn =
                    (from item in List
        ...
        [HttpPost]
        public virtual async Task<IActionResult> PostAsync(UM model)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 14, "Create");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 14, "Delete");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        [HttpPut]
        public virtual async Task<IActionResult> PutAsync(UM model)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 14, "Update");
            if (!AuthForm.WasSuccess)
            {
                return BadRequest(AuthForm.Message);
            }
        ...
        public virtual async Task<IActionResult> UploadAsync(List<UM> list)
        {
            var AuthForm = await _validateSession.GetValidateSession(HttpContext, 14, "Create");
            if (!AuthForm.WasSuccess)
            {
