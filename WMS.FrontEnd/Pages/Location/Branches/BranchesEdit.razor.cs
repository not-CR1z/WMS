...
{
    public partial class BranchesEdit
    {
        ...
        {
            var httpResponse = await Repository.GetAsync<Branch>($"/api/branches/{Id}");
        ...
        {
            var httpResponse = await Repository.PutAsync("/api/branches", Model);
            if (httpResponse.Error)
            {
                var message = await httpResponse.GetErrorMessageAsync();
                await SweetAlertService.FireAsync("Error", message, SweetAlertIcon.Error);
