...
namespace WMS.FrontEnd.Pages.Magister.ProductClassifications
{
    ...
    public partial class ProductClassificationsDetails
    {
        ...
        {
            var url = $"api/productclassificationdetails/totalPages?id={ProductClassificationId}";
        ...
        {
            var url = $"api/productclassificationdetails/getasync?id={ProductClassificationId}&page={page}";
            if (!string.IsNullOrEmpty(Filter))
            {
                url += $"&filter={Filter}";
            }
        ...
        private async Task DeleteAsync(ProductClassificationDetail model)
        {
            ...
            {
                Title = "Confirmación",
                Text = $"¿Realmente deseas eliminar ? {model.Name}",
        ...
        private async Task ShowModal(long id, bool isEdit = false)
        {
            ...
            if (isEdit)
            {
                ...
                modalReference = Modal.Show<ProductClassificationDetailsEdit>(string.Empty, parameters);
            }
            else
            {
                parameters.Add("ProductClassificationId", id);
                modalReference = Modal.Show<ProductClassificationDetailsCreate>(string.Empty, parameters);
            }
