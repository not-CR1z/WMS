...
{
    public partial class ProductTypesIndex
    {
        ...
        {
            var url = $"api/producttypes/getasync?page={page}";
        ...
        {
            var url = $"api/producttypes/totalPages";
            if (!string.IsNullOrEmpty(Filter))
            {
                url += $"?filter={Filter}";
            }
        ...
        private async Task DeleteAsycn(ProductType model)
        {
            ...
            var responseHttp = await Repository.DeleteAsync<ProductType>($"api/producttypes/deleteasync/{model.Id}");
            if (responseHttp.Error)
            {
        ...
        {
            var url = $"api/producttypes/downloadasync";
            if (!string.IsNullOrEmpty(Filter))
            {
                url += $"?filter={Filter}";
            }
